	...
	const posts = getItemsOfType(data, 'post')
		...
		.map(post => ({
			...
			meta: {
				...
				cover: getPostCoverImageId(post),
				imageUrls: []
			},
			frontmatter: {
				...
				metadescription: getMetaDesc(post),
			},
...
function getPostPath(post) {
	return post.link[0].replace("http://frugalisminds.com","");
}
function getPostCategory(post) {
	
	let categ="";
	
	post.category.forEach(match => {
		
		console.log(match);
		let data=match['$'];
		if(data.domain==='post_tag'){
			console.log(data.nicename);
		}else{
			categ=match['_'];
		}
	});
